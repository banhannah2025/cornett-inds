...
import { controlValidationRun, dispatchValidation, getGitHubRateLimit, getValidationRunDetails, listValidationRuns } from "@/lib/legal-ai/github";
...
export async function POST(request: Request) {
  ...
  try {
    if ((body.action === "cancel" || body.action === "rerun") && body.runId) { const result = await controlValidationRun(body.repository, body.runId, body.action); await logLegalAiAudit(`validation-${body.action}`, `${body.action} run ${body.runId}`); return Response.json(result); }
    ...
    const result = await dispatchValidation(body.repository, body.branch, body.scope!); await logLegalAiAudit("validation", `Started ${body.scope} validation on ${body.branch}`); return Response.json(result);
  }
