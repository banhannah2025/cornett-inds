...
export function TravelStayPlanner() {
  ...
  const resolveLocation = async (label: string, latitude?: number, longitude?: number) => { if (latitude !== undefined && longitude !== undefined) return { latitude, longitude }; const response = await fetch(`/api/location-search?q=${encodeURIComponent(label)}`), data = await response.json() as { results?: Place[] }, place = data.results?.[0]; if (!response.ok || !place) throw new Error(); return { latitude: place.latitude, longitude: place.longitude }; };
  const estimateTrip = async (silent = false) => { if (!active?.origin?.trim() || !active.destination.trim()) { if (!silent) setEstimateMessage("Enter both a starting location and destination."); return; } if (!silent) { setEstimating(true); setEstimateMessage("Locating the route…"); } try { const [origin, destination] = await Promise.all([resolveLocation(active.origin, active.originLatitude, active.originLongitude), resolveLocation(active.destination, active.destinationLatitude, active.destinationLongitude)]), radians = (degrees: number) => degrees * Math.PI / 180, dLat = radians(destination.latitude - origin.latitude), dLon = radians(destination.longitude - origin.longitude), a = Math.sin(dLat / 2) ** 2 + Math.cos(radians(origin.latitude)) * Math.cos(radians(destination.latitude)) * Math.sin(dLon / 2) ** 2, miles = 3958.8 * 2 * Math.atan2(Math.sqrt(a), Math.sqrt(1 - a)) * 1.18, vehicle = vehicles.find((item) => item.id === active.vehicleId), mpg = vehicle?.combinedMpg || active.customMpg || 25, roadMode = ["driving", "rv", "motorcycle"].includes(active.travelMode || "driving"), fuel = roadMode ? miles / mpg * (active.fuelPrice ?? 4) : 0, maintenance = roadMode ? miles * ((vehicle?.maintenanceBudget ?? 1200) / 12000) : 0, ownership = roadMode ? miles * (((vehicle?.insuranceBudget ?? 0) + (vehicle?.registrationBudget ?? 0)) / 12000) : 0, total = fuel + maintenance + ownership + (active.otherTravelCost || 0), estimate = { estimatedMiles: Math.round(miles * 100) / 100, estimatedFuelCost: fuel, estimatedMaintenance: maintenance, estimatedOwnership: ownership, estimatedTravelTotal: total }; setPlans((plans) => plans.map((plan) => plan.id === active.id ? { ...plan, ...estimate, originLatitude: origin.latitude, originLongitude: origin.longitude, destinationLatitude: destination.latitude, destinationLongitude: destination.longitude } : plan)); const finance = JSON.parse(localStorage.getItem("blended-basecamp-finance") ?? "{}"), record = { id: active.id, name: `${active.origin} to ${active.destination}`, date: active.arrival, mode: active.travelMode || "driving", vehicleId: active.vehicleId, miles: estimate.estimatedMiles, fuel, maintenance, ownership, other: active.otherTravelCost || 0, total }; localStorage.setItem("blended-basecamp-finance", JSON.stringify({ ...finance, travelEstimates: [...(finance.travelEstimates ?? []).filter((item: { id: number }) => item.id !== active.id), record] })); if (!silent) setEstimateMessage("Trip estimate updated and sent to Money."); } catch { if (!silent) setEstimateMessage("We could not match one of those locations. Select a suggestion or add a more specific city, state, or address."); } finally { if (!silent) setEstimating(false); } };
  useEffect(() => { if (active?.originLatitude !== undefined && active.originLongitude !== undefined && active.destinationLatitude !== undefined && active.destinationLongitude !== undefined) void estimateTrip(true); }, [active?.originLatitude, active?.originLongitude, active?.destinationLatitude, active?.destinationLongitude, active?.travelMode, active?.vehicleId, active?.fuelPrice, active?.customMpg, active?.otherTravelCost]);
  return (
