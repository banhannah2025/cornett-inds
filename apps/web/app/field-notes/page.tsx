...
import { ArrowRight, Heart, MapPinned, Route, ShieldCheck } from "lucide-react";
...
export const metadata: Metadata = {
  title: "Field Notes | Blended Works",
  description:
    "Personal field notes from Robin + Laura™ about faith, family, rebuilding, creativity, work, travel, and the life they are building together.",
};
const icons = {
  "about-us": Heart,
  "our-journey": Route,
  "from-homeless-to-hero": ShieldCheck,
  locations: MapPinned,
};

export default async function FieldNotesPage() {
  const [categories, notes, admin] = await Promise.all([
    ...
    <main>
      <section className="border-b border-[#1e2a24]/10 px-5 py-20 sm:px-8 lg:px-10 lg:py-28">
        <div className="mx-auto grid max-w-7xl gap-10 lg:grid-cols-[0.7fr_1.3fr] lg:items-end">
          <div className="flex flex-wrap items-center gap-4">
            ...
            {admin.isAdmin && <AdminFieldNoteEditor categories={categories} />}
          </div>
          <div>
            ...
            <p className="mt-6 max-w-2xl text-lg leading-8 text-[#59665f]">
              Personal stories about faith, family, rebuilding, creativity,
              work, travel, and the places that become part of our journey.
            </p>
          </div>
        </div>
      </section>
      <section className="px-5 py-16 sm:px-8 lg:px-10 lg:py-24">
        <div className="mx-auto max-w-7xl">
          ...
          <h2 className="mt-2 font-serif text-4xl">Follow the story.</h2>
          <div className="mt-8 grid gap-4 md:grid-cols-2 xl:grid-cols-4">
            {categories.map((category) => {
              const Icon = icons[category.slug as keyof typeof icons] ?? Route;
              return (
                <Link
                ...
                >
                  ...
                  <p className="mt-5 text-xs font-bold uppercase tracking-[0.16em] text-[#a45d2d]">
                    ...
                    {(category.postCount ?? 0) === 1 ? "note" : "notes"}
                  </p>
                </Link>
              );
            })}
      ...
      <section className="bg-[#dfe5dc] px-5 py-16 sm:px-8 lg:px-10 lg:py-24">
        <div className="mx-auto max-w-7xl">
          ...
          <h2 className="mb-10 mt-2 font-serif text-4xl sm:text-5xl">
            From our journal.
          </h2>
          {featured ? (
            <div className="space-y-6">
              <PostCard featured post={featured} routeBase="/field-notes" />
              {remaining.length > 0 && (
