        ...
        <ul className="absolute z-50 mt-1 max-h-64 w-full overflow-y-auto rounded-xl border border-black/10 bg-[#fffdf8] p-1 text-left normal-case tracking-normal shadow-2xl" id={listId} role="listbox">
          {results.map((place) => (
            <li key={place.id} role="option">
              <button className="flex w-full items-start gap-2 rounded-lg px-3 py-2.5 text-left text-xs font-medium leading-5 text-[#1d352f] hover:bg-[#e8efe8]" onClick={() => { onChange(place.label); onSelect?.(place); setOpen(false); }} type="button">
                <MapPin className="mt-0.5 size-4 shrink-0 text-[#a76536]" />
                <span>{place.label}{place.distanceMiles !== undefined ? <small className="mt-0.5 block text-[#6c786f]">{Math.round(place.distanceMiles)} miles away</small> : null}</span>
              </button>
            </li>
          ))}
        </ul>
