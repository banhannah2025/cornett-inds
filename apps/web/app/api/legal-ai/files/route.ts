...
export async function POST(request: Request) {
  ...
  try {
    ...
    await logLegalAiAudit("fileUpload", `Uploaded ${file.name} (${file.size} bytes)`);
...
export async function DELETE(request: Request) {
  ...
  try {
    const client = getSanityWriteClient();
    const file = await client.fetch<{ name: string; assetId?: string } | null>(`*[_type == "legalAiFile" && _id == $id][0]{name,"assetId":asset._ref}`, { id });
    if (!file) return Response.json({ error: "File not found." }, { status: 404 });
    await client.delete(id);
    if (file.assetId) await client.delete(file.assetId).catch(() => undefined);
    await logLegalAiAudit("fileDelete", `Deleted ${file.name}`);
