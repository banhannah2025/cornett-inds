...
import { consumeCodeAiChange, deleteCodeAiDocument, logCodeAiAudit } from "@/lib/code-ai/store";
import {
...
export async function POST(request: Request) {
  ...
  try {
    if (body.action === "commit" && body.branch && body.message && body.changes) { const result = await commitRepositoryFiles({ repository: body.repository, branch: body.branch, message: body.message, changes: body.changes }); if (body.changeSetId?.startsWith("codeAiChangeSet-")) await deleteCodeAiDocument(body.changeSetId); await logCodeAiAudit("commit", `${body.message} · ${body.changes.length} files · ${body.branch}`); return Response.json(result); }
    ...
    const result = await writeRepositoryFile({ repository: body.repository, path: body.path, content: body.content, branch: body.branch || "main", message: body.message }); if (body.changeSetId && body.changeKey) await consumeCodeAiChange(body.changeSetId, body.changeKey); await logCodeAiAudit("fileCommit", `${body.message} · ${body.path}`); return Response.json(result);
  } catch (error) {
