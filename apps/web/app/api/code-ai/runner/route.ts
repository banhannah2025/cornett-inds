...
export async function POST(request: Request) {
  if (!(await getCodeAiOwner())) return Response.json({ error: "Forbidden" }, { status: 403 });
  const body = (await request.json().catch(() => null)) as { action?: "start" | "cancel" | "rerun"; repository?: string; branch?: string; scope?: "web" | "all"; runId?: number; approved?: boolean } | null;
  ...
  try {
    ...
    if (!body.branch || !["web", "all"].includes(body.scope ?? "")) return Response.json({ error: "Branch and validation scope are required." }, { status: 400 });
