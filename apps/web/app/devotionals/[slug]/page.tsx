...
export async function generateMetadata({
  params,
}: PageProps<"/devotionals/[slug]">): Promise<Metadata> {
  const { slug } = await params;
  const devotional = await getDevotional(slug);
  return devotional
    ? {
        title: devotional.seoTitle ?? `${devotional.title} | Daily Devotionals`,
        description: devotional.seoDescription ?? devotional.excerpt,
      }
...
export default async function DevotionalPage({
  params,
}: PageProps<"/devotionals/[slug]">) {
  const { slug } = await params;
  const [devotional, admin] = await Promise.all([
    getDevotional(slug),
    getAdminContext(),
  ]);
      ...
      <article>
          ...
          <div className="mx-auto max-w-4xl">
            <div className="flex flex-wrap items-center justify-between gap-4">
              <Link
                ...
                href="/devotionals"
              >
                <ArrowLeft className="size-4" /> All devotionals
              </Link>
              {admin.isAdmin && (
                <AdminDevotionalEditor
                  devotional={{
                    ...
                    bodyText: portableTextToEditorText(devotional.body),
                  }}
                />
              )}
            </div>
          ...
          <div className="relative mx-auto aspect-[16/8] max-w-7xl overflow-hidden bg-[#dfe5dc] sm:rounded-[2.5rem]">
            ...
            {admin.isAdmin && (
              <div className="absolute right-5 top-5 z-10">
                <AdminImageEditor
                  assets={imageAssets}
                  currentAlt={devotional.mainImage?.alt}
                  currentCaption={devotional.mainImage?.caption}
                  documentId={devotional._id}
                  documentType="devotional"
                  hasImage={Boolean(devotional.mainImage?.asset?._ref)}
                />
              </div>
            )}
          </div>
        ...
        <div className="mx-auto max-w-3xl px-5 py-16 sm:px-8 lg:py-24">
          <section className="rounded-[2rem] bg-[#e5e9df] p-8 sm:p-10">
            ...
            <blockquote className="mt-5 whitespace-pre-line font-serif text-2xl italic leading-relaxed text-[#35443c] sm:text-3xl">
              &ldquo;{devotional.scriptureText}&rdquo;
            </blockquote>
          </section>
          {devotional.body?.length ? (
            <PortableText components={components} value={devotional.body} />
          ) : (
            <p className="mt-8 text-lg leading-8 text-[#59665f]">
              This reflection is being prepared for publication.
            </p>
          )}
          {devotional.prayer && (
            <section className="mt-14 border-t border-[#1e2a24]/15 pt-10">
              ...
              <p className="mt-5 whitespace-pre-line font-serif text-2xl italic leading-relaxed text-[#39473f]">
                {devotional.prayer}
              </p>
            </section>
          )}
